...
const LevelCompleteScreen: React.FC<LevelCompleteScreenProps> = ({ mpsEarned, onNext, isTestingEditorLevel, onReturnToEditor }) => {
  ...
  return (
    <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center z-30 font-sans">
      <div className="text-center p-10 bg-gray-800 rounded-lg shadow-xl border-2 border-green-500">
        <h2 className="text-3xl sm:text-5xl font-black tracking-tighter text-green-400 mb-4">LEVEL COMPLETE!</h2>
        <p className="text-xl sm:text-2xl text-white mb-6">You earned <span className="font-bold text-yellow-400">{mpsEarned}</span> $MPS!</p>
        <button
          onClick={handleClick}
